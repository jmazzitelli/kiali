...
minikube_install_basic_demo() {
  ...
  if [ "${KIALI_VERSION}" == "dev" ]; then
    ...
    helm --kube-context ${CLUSTER1_ISTIO} upgrade --install --namespace istio-system --version ${KIALI_VERSION} --set auth.strategy=anonymous --set deployment.service.type=LoadBalancer kiali-server kiali/kiali-server
  fi
